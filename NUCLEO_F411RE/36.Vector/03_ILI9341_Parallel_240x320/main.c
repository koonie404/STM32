...
typedef enum {
    EXPR_NORMAL, EXPR_HAPPY, EXPR_SAD, EXPR_ANGRY,EXPR_ANGRY_STERN,
...
static void Eye_Angry_V2(int16_t cx, uint8_t is_left) {
    // 1. 위치 계산 (Eye_Half와 동일한 방식)
    int16_t sx = EYE_AREA_X + cx - EYE_W/2;
    int16_t sy = EYE_AREA_Y + CY - EYE_H/2;
    ...
    // 3. 화난 눈매를 위해 윗부분을 사선으로 깎기 (배경색으로 덮기)
    // i는 깎아낼 높이입니다.
    for (int16_t i = 0; i < 30; i++) {
        if (is_left) {
            // 왼쪽 눈: 오른쪽(미간)이 낮아지도록 / 모양으로 깎음
            // x 좌표를 점점 오른쪽으로 밀면서 가로선을 긋습니다.
            LCD_HLineFast(sx + i, sy + i, EYE_W, EYE_BG);
        } else {
            // 오른쪽 눈: 왼쪽(미간)이 낮아지도록 \ 모양으로 깎음
            // 너비를 점점 줄이면서 가로선을 긋습니다.
            LCD_HLineFast(sx, sy + i, EYE_W - i, EYE_BG);
        }
    }
}
...
static void Draw_Expression(Expression_t expr, int16_t ox, int16_t oy) {
    ...
    switch(expr) {
        case EXPR_NORMAL:
            ...
            Eye_Normal(RX, ox, oy);
            break;
        case EXPR_ANGRY_STERN:
			Eye_Angry_V2(LX, 1); // 왼쪽 눈 (치켜뜸)
			Eye_Angry_V2(RX, 0); // 오른쪽 눈 (치켜뜸)
			break;
        case EXPR_HAPPY:
            Eye_Happy(LX);
            Eye_Happy(RX);
...
static void Anim_Demo(void) {
    ...
    Anim_SetExpr(EXPR_ANGRY_STERN); HAL_Delay(1000);
